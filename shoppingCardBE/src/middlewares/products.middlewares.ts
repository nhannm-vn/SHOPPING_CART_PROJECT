      ...
      medias: {
        ...
        custom: {
          ...
          options: (value: Media[], { req }) => {
            //_Lấy danh sách key của type từ MediaType
            //nghĩa là lấy những key nào mà kép về số thì nó là số
            const MediaTypeArray = Object.keys(MediaType).filter((key) => !isNaN(Number(key)))
            //_Có danh sách rồi thì mình bắt đầu kiểm tra
            if (
              //_Nếu chỉ cần 1 trong các item có url không là string hoặc type không hợp lí thì báo lỗi
              value.some((item: any) => {
