...
export default class RefreshToken {
  ...
  constructor({ _id, token, created_at, user_id, iat, exp }: RefreshTokenType) {
    ...
    this.iat = new Date(iat * 1000) //iat mà mình nhận được tên là epoch time nên * 1000 mới chuyển về được
    this.exp = new Date(exp * 1000) //iat mà mình nhận được tên là epoch time nên * 1000 mới chuyển về được
  }
}
